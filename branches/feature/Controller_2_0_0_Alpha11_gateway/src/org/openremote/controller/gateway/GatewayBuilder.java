...
/**
 * Generic interface for all gateway implementations. <p>
 *
 * The gateway builder interface is responsible for taking the XML definition from the
 * controller.xml file and turning it into Java object instances (which must in turn
 * implement the gateway interface).  <p>
 *
 * The generic structure of all gateway XML definitions is as follows:
 *
 * <pre>{@code
 ...
 * @see Gateway
 *
 * Code adapted from Command defition
 */
public interface GatewayBuilder
{
  ...
  /**
   * String constant for the top level gateway element protocol attribute: ("{@value}")
   */
