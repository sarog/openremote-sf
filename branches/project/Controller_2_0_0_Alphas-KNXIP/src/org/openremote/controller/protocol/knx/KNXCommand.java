...
abstract class KNXCommand implements Command {
   ...
   /**
    * Factory method for creating KNX command instances {@link GroupValueWrite} and {@link GroupValueRead} based on a
    * human-readable configuration strings.
    * <p>
    * 
    * Each KNX command instance is associated with a link to a connection manager and a destination group address.
    * 
    * @param name
    *           Command lookup name. This is usually a human-readable string used in configuration and tools. Note that
    *           multiple lookup names can be used to return Java equal() (but not same instance) commands.
    * @param dpt
    *           KNX datapoint type associated with this command
    * @param mgr
    *           KNX connection manager used to transmit this command
    * @param address
    *           KNX destination group address.
    * @param parameter
    *           parameter for this command or <tt>null</tt> if not available
    * 
    ...
    */
   static KNXCommand createCommand(String name, DataPointType dpt, KNXIpConnectionManager mgr, GroupAddress address,
         CommandParameter parameter) {
      name = name.trim().toUpperCase();

      GroupValueWrite writeCmd = GroupValueWrite.createCommand(name, dpt, mgr, address, parameter);

      if (writeCmd != null) return writeCmd;

      GroupValueRead readCmd = GroupValueRead.createCommand(name, mgr, address, dpt);

      if (readCmd != null) return readCmd;
   ...
   /**
    * Constructs a KNX command with a given connection manager, destination group address and an APDU payload.
    * 
    * @param connectionManager
    *           KNX connection manager instance used for transmitting this commnad
    * @param address
    ...
    *           KNX datapoint type associated with this command
    */
   KNXCommand(KNXIpConnectionManager connectionManager, GroupAddress address, ApplicationProtocolDataUnit apdu,
         DataPointType dpt) {
   ...
   public String toString() {
      ...
      String sourceAddr = IndividualAddress.formatToAreaLineDevice(new byte[] { frame[CEMI_SOURCEADDR_HIGH_OFFSET],
            frame[CEMI_SOURCEADDR_LOW_OFFSET] });

      String destAddr = GroupAddress.formatToMainMiddleSub(new byte[] { frame[CEMI_DESTADDR_HIGH_OFFSET],
            frame[CEMI_DESTADDR_LOW_OFFSET] });

      String data = apdu.dataAsString();

      buffer.append("[FRAME] ").append(msgCode).append(" ").append(sourceAddr).append(" -> ").append(destAddr).append(
            " Data: ").append(data);
    ...
    *           KNX read command
    * 
    * @return Returns the application protocol data unit (APDU) for a Group Value Read Response frame. This frame
    *         contains the response value from the device.
    *         <P>
    * 
    *         NOTE: may return <code>null</code> in case there's a connection exception or the read response is not
    *         available from the device yet.
    */
   ...
   /**
    * The KNX Common External Message Interface (a.k.a cEMI) frame has a variable length and structure depending on the
    * Common EMI frame message code (first byte) and additional info length (second byte).
    * <p>
    * 
    * In a very generic fashion, a Common EMI frame can be defined as follows (KNX 1.1 Application Note 033 - Common EMI
    * Specification, 2.4 Basic Message Structure, page 8):
    * 
    * <pre>
    * @code
    * 
    ...
    * </pre>
    * 
    * KNX communication stack defines a frame transfer service (known as L_Data Service) in the data link layer (KNX 1.1
    * -- Volume 3 System Specification, Part 2 Communication, Chapter 2 Data Link Layer General, section 2.1 L_Data
    * Service, page 8).
    * <p>
    * 
    * Link layer data services are available in "normal" mode (vs. bus monitor mode). A data request (known as
    * L_Data.req primitive) is used to transmit a frame. The corresponding Common EMI frame for L_Data.req is defined as
    * shown below (KNX 1.1 Application Note 033, section 2.5.33 L_Data.req, page 13). Example assumes a standard
    * (non-extended) frame with no additional info fields set in the frame. The application protocol data unit (APDU) is
    ...
    * <p>
    * 
    * Common External Message Interface (EMI) defines two control fields in its frame format (one byte each). The bit
    * structure of each control field is defined in the KNX 1.1 Application Note 033: Common EMI Specification, section
    * 2.4 Basic Message Structure:
    * 
    * 
    * <pre>
    * @code
   ...
   Byte[] getCEMIFrame() {
      ...
      /*
       * Destination Address Type bit for group address in the second control field of the common EMI frame - most
      ...
      /*
       * Non-extended frame format in the second control field of the common EMI frame(four zero bits)
       */
      ...
      cemi.add( // Control Field 1
            (byte) (STANDARD_FRAME_TYPE + REPEAT_FRAME + SYSTEM_BROADCAST + NORMAL_PRIORITY + REQUEST_ACK));

      cemi.add( // Control Field 2
            (byte) (GROUP_ADDRESS + HOP_COUNT + NON_EXTENDED_FRAME_FORMAT));
      ...
      if (protocolDataUnit.length > 2) {
         ...
         if (apduDataLength != protocolDataUnit.length - 1) {
            throw new Error("APDU reported data length does not match the actual data length : " + apduDataLength
                  + " != " + (protocolDataUnit.length - 1) + "(" + this.apdu + ").");
         }
