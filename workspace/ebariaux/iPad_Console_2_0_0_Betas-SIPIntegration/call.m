...
//  NSNumber *value;
  ...
  if (ci.state == PJSIP_INV_STATE_DISCONNECTED) // Session is terminated.
  {
    ...
    sip_ring_stop([[((AppDelegate *)[[UIApplication sharedApplication] delegate]) sipController] pjsipConfig]);
  ...
  else if (ci.state == PJSIP_INV_STATE_EARLY) 
  {
        ...
        ci.media_status == PJSUA_CALL_MEDIA_NONE) 
    {
      ...
      sip_ringback_start([[((AppDelegate *)[[UIApplication sharedApplication] delegate]) sipController] pjsipConfig]);
    }
	}
           ...
           pjsip_rx_data *rdata)
{
  ...
  /* FIXME: Start ringback */
  sip_ring_start([[((AppDelegate *)[[UIApplication sharedApplication] delegate]) sipController] pjsipConfig]);
...
//EBR    [app displayParameterError:     @"Invalid value for Local Port (expecting 1-65535)."];
...
//EBR    [app displayParameterError:     @"Invalid value for RTP port (expecting 1-65535)."];
    status = PJ_EINVAL;
    goto error;
  }
...
pj_status_t sip_connect(pj_pool_t *pool, pjsua_acc_id *acc_id)
{
  ...
  if ((status = pjsua_verify_sip_url(acc_cfg.id.ptr)) != 0) 
  {
    PJ_LOG(1,(THIS_FILE, "Error: invalid SIP URL '%s' in local id argument", 
      acc_cfg.id));
// EBR    [app displayParameterError: @"Invalid value for username or server."];
    return status;
  }
  ...
  if ((status = pjsua_verify_sip_url(acc_cfg.reg_uri.ptr)) != 0) 
  {
    PJ_LOG(1,(THIS_FILE,  "Error: invalid SIP URL '%s' in registrar argument",
      acc_cfg.reg_uri));
//EBR    [app displayParameterError: @"Invalid value for server parameter."];
    return status;
  }
...
//EBR    [app displayParameterError: @"Invalid value for timeout (expecting 1-3600)."];
    return PJ_EINVAL;
  }
  ...
  while (anObject = [enumerator nextObject])
  {
    ...
    if (proxy && strlen(proxy))
    {
      ...
      if ((status = pjsua_verify_sip_url(pj_proxy.ptr)) != 0) 
      {
        ...
        [mutableStr release];
//EBR        [app displayParameterError: @"Invalid value for proxy parameter."];
        continue;
      }
...
//EBR    [app displayParameterError: @"Error adding new account."];
  }
#endif  
  return status;
}
