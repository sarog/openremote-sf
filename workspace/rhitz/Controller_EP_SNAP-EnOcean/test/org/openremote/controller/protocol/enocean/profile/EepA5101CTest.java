...
public class EepA5101CTest
{
  ...
  @Test public void testBasicConstruction() throws Exception
  {
    ...
    Eep eep = EepType.lookup("A5-10-1C").createEep(
        deviceID, Constants.TEMPERATURE_STATUS_COMMAND
    );

    Assert.assertTrue(eep instanceof EepA5101C);
    Assert.assertEquals(EepType.EEP_TYPE_A5101C, eep.getType());

    // Old EEP number ...

    eep = EepType.lookup("07-10-1C").createEep(
        deviceID, Constants.TEMPERATURE_STATUS_COMMAND
    );
  ...
  @Test public void testUpdateIllumination() throws Exception
  {
    ...
    EepA5101C eep = (EepA5101C)EepType.lookup("A5-10-1C").createEep(
  ...
  {
    EepA5101C eep = (EepA5101C)EepType.lookup("A5-10-1C").createEep(
        deviceID, Constants.ILL_SET_POINT_STATUS_COMMAND
    );
    ...
    Assert.assertTrue(isUpdate);
    Assert.assertEquals(Double.valueOf(1000), eep.getSetPoint());
  ...
  private Esp34BSTelegram createRadioTelegram(DeviceID deviceID, int rawIlluminationValue, int rawSetPointValue,
