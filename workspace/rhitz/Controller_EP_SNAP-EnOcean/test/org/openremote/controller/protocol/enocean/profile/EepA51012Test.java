...
/**
 * Unit tests for {@link EepA51012} class.
 *
 * @author Rainer Hitz
 */
public class EepA51012Test
{
  ...
  @Test public void testBasicConstruction() throws Exception
  {
    ...
    Eep eep = EepType.lookup("A5-10-12").createEep(
        deviceID, Constants.TEMPERATURE_STATUS_COMMAND
    );

    Assert.assertTrue(eep instanceof EepA51012);
    Assert.assertEquals(EepType.EEP_TYPE_A51012, eep.getType());

    // Old EEP number ...

    eep = EepType.lookup("07-10-12").createEep(
        deviceID, Constants.TEMPERATURE_STATUS_COMMAND
    );
  ...
  @Test public void testUpdateSetPoint() throws Exception
  {
    ...
    EepA51012 eep = (EepA51012)EepType.lookup("A5-10-12").createEep(
    ...
    Esp34BSTelegram telegram = createRadioTelegram(
        deviceID, rawSetPointValue, rawHumidityValue, rawTempValue, isTeachIn
    );
  ...
  @Test public void testInvalidDeviceID() throws Exception
  {
    ...
    Esp34BSTelegram telegram = createRadioTelegram(
        invalidDeviceID, rawSetPointValue, rawHumidityValue, rawTempValue, isTeachIn
    );
  ...
  private Esp34BSTelegram createRadioTelegram(DeviceID deviceID, int rawSetPointValue, int rawHumidityValue,
