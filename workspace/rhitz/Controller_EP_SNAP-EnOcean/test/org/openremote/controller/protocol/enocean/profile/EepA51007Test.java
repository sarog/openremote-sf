...
/**
 * Unit tests for {@link EepA51007} class.
 *
 * @author Rainer Hitz
 */
public class EepA51007Test
{
  ...
  @Test public void testBasicConstruction() throws Exception
  {
    ...
    Eep eep = EepType.lookup("A5-10-07").createEep(
        deviceID, Constants.TEMPERATURE_STATUS_COMMAND
    );

    Assert.assertTrue(eep instanceof EepA51007);
    Assert.assertEquals(EepType.EEP_TYPE_A51007, eep.getType());

    // Old EEP number ...

    eep = EepType.lookup("07-10-07").createEep(
        deviceID, Constants.TEMPERATURE_STATUS_COMMAND
    );
  ...
  {
    EepA51007 eep = (EepA51007)EepType.lookup("A5-10-07").createEep(
    ...
    EspRadioTelegram telegram = createRadioTelegramESP3(
        deviceID, rawFanSpeedValue, rawTempValue, isTeachIn
    );
  ...
  @Test public void testInvalidDeviceID() throws Exception
  {
    ...
    EspRadioTelegram telegram = createRadioTelegramESP3(
        invalidDeviceID, rawFanSpeedValue, rawTempValue, isTeachIn
    );
