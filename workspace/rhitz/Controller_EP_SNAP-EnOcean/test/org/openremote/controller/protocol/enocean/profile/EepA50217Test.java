...
/**
 * Unit tests for {@link EepA50217} class.
 *
 * @author Rainer Hitz
 */
public class EepA50217Test extends EepA502XXTest
{
  ...
  @Test public void testBasicConstruction() throws Exception
  {
    ...
    Eep eep = EepType.lookup("A5-02-17").createEep(
        deviceID, Constants.TEMPERATURE_STATUS_COMMAND
    );

    Assert.assertTrue(eep instanceof EepA50217);
    Assert.assertEquals(EepType.EEP_TYPE_A50217, eep.getType());

    // Old EEP number ...

    eep = EepType.lookup("07-02-17").createEep(
        deviceID, Constants.TEMPERATURE_STATUS_COMMAND
    );
  ...
  {
    EepA50217 eep = (EepA50217)EepType.lookup("A5-02-17").createEep(
        deviceID, Constants.TEMPERATURE_STATUS_COMMAND
    );
    ...
    Assert.assertTrue(isUpdate);
    Assert.assertEquals(Double.valueOf(90), eep.getTemperature(), 0.0);
  }
