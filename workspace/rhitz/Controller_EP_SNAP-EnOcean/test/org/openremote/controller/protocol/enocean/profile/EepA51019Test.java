...
/**
 * Unit tests for {@link EepA51019} class.
 *
 * @author Rainer Hitz
 */
public class EepA51019Test
{
  ...
  @Test public void testBasicConstruction() throws Exception
  {
    ...
    Eep eep = EepType.lookup("A5-10-19").createEep(
        deviceID, Constants.TEMPERATURE_STATUS_COMMAND
    );

    Assert.assertTrue(eep instanceof EepA51019);
    Assert.assertEquals(EepType.EEP_TYPE_A51019, eep.getType());

    // Old EEP number ...

    eep = EepType.lookup("07-10-19").createEep(
        deviceID, Constants.TEMPERATURE_STATUS_COMMAND
    );
  ...
  {
    EepA51019 eep = (EepA51019)EepType.lookup("A5-10-19").createEep(
  ...
  private Esp34BSTelegram createRadioTelegramESP3(DeviceID deviceID, int rawHumidityValue, int rawSetPointValue,
                                                  int rawTempValue, int rawFanSpeedValue, boolean isOccupancyEnabled,
                                                  boolean isOccupied, boolean isTeachIn)
  {
    byte[] payload = new byte[4];
    payload[0] = (byte)rawHumidityValue;
  ...
  private Esp24BSTelegram createRadioTelegramESP2(DeviceID deviceID, int rawHumidityValue, int rawSetPointValue,
                                                  int rawTempValue, int rawFanSpeedValue, boolean isOccupancyEnabled,
                                                  boolean isOccupied, boolean isTeachIn)
  {
    byte[] payload = new byte[4];
