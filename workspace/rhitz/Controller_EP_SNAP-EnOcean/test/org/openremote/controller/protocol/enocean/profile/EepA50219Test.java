...
/**
 * Unit tests for {@link EepA50219} class.
 *
 * @author Rainer Hitz
 */
public class EepA50219Test extends EepA502XXTest
{
  ...
  @Test public void testBasicConstruction() throws Exception
  {
    ...
    Eep eep = EepType.lookup("A5-02-19").createEep(
        deviceID, Constants.TEMPERATURE_STATUS_COMMAND
    );

    Assert.assertTrue(eep instanceof EepA50219);
    Assert.assertEquals(EepType.EEP_TYPE_A50219, eep.getType());

    // Old EEP number ...

    eep = EepType.lookup("07-02-19").createEep(
        deviceID, Constants.TEMPERATURE_STATUS_COMMAND
    );
  ...
  {
    EepA50219 eep = (EepA50219)EepType.lookup("A5-02-19").createEep(
        deviceID, Constants.TEMPERATURE_STATUS_COMMAND
    );
    ...
    Assert.assertTrue(isUpdate);
    Assert.assertEquals(Double.valueOf(110), eep.getTemperature(), 0.0);
  }
