...
/**
 * Unit tests for {@link EepA50212} class.
 *
 * @author Rainer Hitz
 */
public class EepA50212Test extends EepA502XXTest
{
  ...
  @Test public void testBasicConstruction() throws Exception
  {
    ...
    Eep eep = EepType.lookup("A5-02-12").createEep(
        deviceID, Constants.TEMPERATURE_STATUS_COMMAND
    );

    Assert.assertTrue(eep instanceof EepA50212);
    Assert.assertEquals(EepType.EEP_TYPE_A50212, eep.getType());

    // Old EEP number ...

    eep = EepType.lookup("07-02-12").createEep(
        deviceID, Constants.TEMPERATURE_STATUS_COMMAND
    );
  ...
  {
    EepA50212 eep = (EepA50212)EepType.lookup("A5-02-12").createEep(
        deviceID, Constants.TEMPERATURE_STATUS_COMMAND
    );
