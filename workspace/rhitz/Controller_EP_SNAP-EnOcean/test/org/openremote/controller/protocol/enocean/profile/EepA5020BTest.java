...
/**
 * Unit tests for {@link EepA5020B} class.
 *
 * @author Rainer Hitz
 */
public class EepA5020BTest extends EepA502XXTest
{
  ...
  @Test public void testBasicConstruction() throws Exception
  {
    ...
    Eep eep = EepType.lookup("A5-02-0B").createEep(
        deviceID, Constants.TEMPERATURE_STATUS_COMMAND
    );

    Assert.assertTrue(eep instanceof EepA5020B);
    Assert.assertEquals(EepType.EEP_TYPE_A5020B, eep.getType());

    // Old EEP number ...

    eep = EepType.lookup("07-02-0B").createEep(
        deviceID, Constants.TEMPERATURE_STATUS_COMMAND
    );
  ...
  {
    EepA5020B eep = (EepA5020B)EepType.lookup("A5-02-0B").createEep(
        deviceID, Constants.TEMPERATURE_STATUS_COMMAND
    );
    ...
    Assert.assertTrue(isUpdate);
    Assert.assertEquals(Double.valueOf(100), eep.getTemperature(), 0.0);
  }
