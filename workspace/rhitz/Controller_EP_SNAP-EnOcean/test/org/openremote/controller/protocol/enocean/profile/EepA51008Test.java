...
/**
 * Unit tests for {@link EepA51008} class.
 *
 * @author Rainer Hitz
 */
public class EepA51008Test
{
  ...
  @Test public void testBasicConstruction() throws Exception
  {
    ...
    Eep eep = EepType.lookup("A5-10-08").createEep(
        deviceID, Constants.TEMPERATURE_STATUS_COMMAND
    );

    Assert.assertTrue(eep instanceof EepA51008);
    Assert.assertEquals(EepType.EEP_TYPE_A51008, eep.getType());

    // Old EEP number ...

    eep = EepType.lookup("07-10-08").createEep(
        deviceID, Constants.TEMPERATURE_STATUS_COMMAND
    );
  ...
  {
    EepA51008 eep = (EepA51008)EepType.lookup("A5-10-08").createEep(
    ...
    EspRadioTelegram telegram = createRadioTelegramESP3(
        deviceID, rawFanSpeedValue, rawTempValue, isOccupied, isTeachIn
    );
  ...
  @Test public void testInvalidDeviceID() throws Exception
  {
    ...
    EspRadioTelegram telegram = createRadioTelegramESP3(
        invalidDeviceID, rawFanSpeedValue, rawTempValue, isOccupied, isTeachIn
    );
  ...
  private Esp34BSTelegram createRadioTelegramESP3(DeviceID deviceID, int rawFanSpeedValue,
  ...
  private Esp24BSTelegram createRadioTelegramESP2(DeviceID deviceID, int rawFanSpeedValue,
                                                  int rawTempValue, boolean isOccupancy, boolean isTeachIn)
  {
    byte[] payload = new byte[4];
    payload[0] = (byte)rawFanSpeedValue;
