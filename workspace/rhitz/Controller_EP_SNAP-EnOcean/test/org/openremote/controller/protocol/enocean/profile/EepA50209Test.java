...
/**
 * Unit tests for {@link EepA50209} class.
 *
 * @author Rainer Hitz
 */
public class EepA50209Test extends EepA502XXTest
{
  ...
  @Test public void testBasicConstruction() throws Exception
  {
    ...
    Eep eep = EepType.lookup("A5-02-09").createEep(
        deviceID, Constants.TEMPERATURE_STATUS_COMMAND
    );

    Assert.assertTrue(eep instanceof EepA50209);
    Assert.assertEquals(EepType.EEP_TYPE_A50209, eep.getType());

    // Old EEP number ...

    eep = EepType.lookup("07-02-09").createEep(
        deviceID, Constants.TEMPERATURE_STATUS_COMMAND
    );
  ...
  {
    EepA50209 eep = (EepA50209)EepType.lookup("A5-02-09").createEep(
        deviceID, Constants.TEMPERATURE_STATUS_COMMAND
    );
    ...
    Assert.assertTrue(isUpdate);
    Assert.assertEquals(Double.valueOf(80), eep.getTemperature(), 0.0);
  }
