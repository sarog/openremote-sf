...
/**
 * Unit tests for {@link EepA50202} class.
 *
 * @author Rainer Hitz
 */
public class EepA50202Test extends EepA502XXTest
{
  ...
  @Test public void testBasicConstruction() throws Exception
  {
    ...
    Eep eep = EepType.lookup("A5-02-02").createEep(
        deviceID, Constants.TEMPERATURE_STATUS_COMMAND
    );

    Assert.assertTrue(eep instanceof EepA50202);
    Assert.assertEquals(EepType.EEP_TYPE_A50202, eep.getType());

    // Old EEP number ...

    eep = EepType.lookup("07-02-02").createEep(
        deviceID, Constants.TEMPERATURE_STATUS_COMMAND
    );
  ...
  {
    EepA50202 eep = (EepA50202)EepType.lookup("A5-02-02").createEep(
        deviceID, Constants.TEMPERATURE_STATUS_COMMAND
    );
