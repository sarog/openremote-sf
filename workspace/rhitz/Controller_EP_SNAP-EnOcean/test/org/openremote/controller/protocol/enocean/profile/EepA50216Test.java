...
public class EepA50216Test extends EepA502XXTest
{
  ...
  @Test public void testBasicConstruction() throws Exception
  {
    ...
    Eep eep = EepType.lookup("A5-02-16").createEep(
        deviceID, Constants.TEMPERATURE_STATUS_COMMAND
    );

    Assert.assertTrue(eep instanceof EepA50216);
    Assert.assertEquals(EepType.EEP_TYPE_A50216, eep.getType());

    // Old EEP number ...

    eep = EepType.lookup("07-02-16").createEep(
        deviceID, Constants.TEMPERATURE_STATUS_COMMAND
    );
  ...
  {
    EepA50216 eep = (EepA50216)EepType.lookup("A5-02-16").createEep(
        deviceID, Constants.TEMPERATURE_STATUS_COMMAND
    );
