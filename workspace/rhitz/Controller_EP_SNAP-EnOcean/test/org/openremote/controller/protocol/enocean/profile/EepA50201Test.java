 ...
 * @author Rainer Hitz
 */
public class EepA50201Test extends EepA502XXTest
{
  ...
  @Test public void testBasicConstruction() throws Exception
  {
    ...
    Eep eep = EepType.lookup("A5-02-01").createEep(
        deviceID, Constants.TEMPERATURE_STATUS_COMMAND
    );

    Assert.assertTrue(eep instanceof EepA50201);
    Assert.assertEquals(EepType.EEP_TYPE_A50201, eep.getType());

    // Old EEP number ...

    eep = EepType.lookup("07-02-01").createEep(
        deviceID, Constants.TEMPERATURE_STATUS_COMMAND
    );
  ...
  {
    EepA50201 eep = (EepA50201)EepType.lookup("A5-02-01").createEep(
        deviceID, Constants.TEMPERATURE_STATUS_COMMAND
    );
