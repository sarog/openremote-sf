...
/**
 * Represents the EnOcean equipment profile (EEP) 'A5-10-19'. <p>
 *
 * <pre>
 *
 *     +------+------+------------------------------------------------------------------+
 ...
 *     +------+------+------------------------------------------------------------------+
 *     | TYPE |  19  |Humidity, Temperature Set Point, Temperature Sensor, Fan Speed    |
 *     |      |      |                    and Occupancy Control                         |
 *     +------+------+------------------------------------------------------------------+
 *
 * </pre>
 *
 * The 'A5-10-19' profile data is transmitted by means of 4BS radio telegrams.
 ...
 *                                                                    | | | | |
 *            +-------------------------------------------------------+ +-+ + +
 *            |    Humidity   | Temp Setpoint |  Temperature  | |Fan  | | | | |
 *            |      HUM      |      TMPSP    |      TMP      | |Speed| | | | |
 *            +-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+
 ...
 *     +--------------------------------------------------------------------------------------------------+
 *     |0     |8   |DB3.7..DB3.0|Humidity      |  HUM   |Rel. Humidity(linear)|   0..250  | 0..100   |%   |
 *     +--------------------------------------------------------------------------------------------------+
 ...
 * @author Rainer Hitz
 */
public class EepA51019 extends EepA510XX
{
   ...
   */
  static final int EEP_A51019_HUM_OFFSET = 0;

  /**
   * Bit offset of 'temperature set point' data field.
   */
  static final int EEP_A51019_TMPSP_OFFSET = 8;

  /**
   * Bit offset of temperature data field.
   */
  static final int EEP_A51019_TMP_OFFSET = 16;

  /**
   * Bit offset of 'fan speed' data field.
   */
  static final int EEP_A51019_FAN_OFFSET = 25;

  /**
   * Bit offset of 'occupancy enable/disable' data field.
   */
  static final int EEP_A51019_OED_OFFSET = 30;

  /**
   * Bit offset of 'occupancy button' data field.
   */
  static final int EEP_A51019_OB_OFFSET = 31;


  // Constructors ---------------------------------------------------------------------------------

  public EepA51019(DeviceID deviceID, String commandString) throws ConfigurationException
  {
    super(EepType.EEP_TYPE_A51019, deviceID);

    initCommand(commandString);

    this.humidity = createHumidityRange(EEP_A51019_HUM_OFFSET);
    this.setPoint = createTempSetPointRange(EEP_A51019_TMPSP_OFFSET);
    this.temperature = createTemperatureRange8Bit3(EEP_A51019_TMP_OFFSET);
    this.fanSpeed = createFanSpeedRange2(EEP_A51019_FAN_OFFSET);
    this.occupancyEnabled = createOccupancyButtonEnableDisableBool(EEP_A51019_OED_OFFSET);
    this.occupancy = createOccupancyButtonBool(EEP_A51019_OB_OFFSET);

    this.sensorData = new EepData(
        EepType.EEP_TYPE_A51019, 4, humidity, setPoint,
