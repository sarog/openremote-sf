...
/**
 * Represents the EnOcean equipment profile (EEP) 'A5-10-18'. <p>
 *
 * <pre>
 *
 *     +------+------+------------------------------------------------------------------+
 ...
 *     +------+------+------------------------------------------------------------------+
 *     | TYPE |  18  |Illumination, Temperature Set Point, Temperature Sensor, Fan Speed|
 *     |      |      |                    and Occupancy Control                         |
 *     +------+------+------------------------------------------------------------------+
 *
 * </pre>
 *
 * The 'A5-10-18' profile data is transmitted by means of 4BS radio telegrams.
 ...
 *                                                                    | | | | |
 *            +-------------------------------------------------------+ +-+ + +
 *            |  Illumination | Temp Setpoint |  Temperature  | |Fan  | | | | |
 *            |      ILL      |      TMPSP    |      TMP      | |Speed| | | | |
 *            +-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+
 ...
 *     +-------------------------------------------------------------------------------------------------+
 *     |0     |8   |DB3.7..DB3.0|Illumination  |  ILL   |Set point(linear)   |   0..250  | 0..1000  |lx  |
 *     +-------------------------------------------------------------------------------------------------+
 ...
 * @author Rainer Hitz
 */
public class EepA51018 extends EepA510XX
{
   ...
   */
  static final int EEP_A51018_ILL_OFFSET = 0;

  /**
   * Bit offset of 'temperature set point' data field.
   */
  static final int EEP_A51018_TMPSP_OFFSET = 8;

  /**
   * Bit offset of temperature data field.
   */
  static final int EEP_A51018_TMP_OFFSET = 16;

  /**
   * Bit offset of 'fan speed' data field.
   */
  static final int EEP_A51018_FAN_OFFSET = 25;

  /**
   * Bit offset of 'occupancy enable/disable' data field.
   */
  static final int EEP_A51018_OED_OFFSET = 30;

  /**
   * Bit offset of 'occupancy button' data field.
   */
  static final int EEP_A51018_OB_OFFSET = 31;


  // Constructors ---------------------------------------------------------------------------------

  public EepA51018(DeviceID deviceID, String commandString) throws ConfigurationException
  {
    super(EepType.EEP_TYPE_A51018, deviceID);

    initCommand(commandString);

    this.illumination = createIlluminationRange(EEP_A51018_ILL_OFFSET);
    this.setPoint = createTempSetPointRange(EEP_A51018_TMPSP_OFFSET);
    this.temperature = createTemperatureRange8Bit3(EEP_A51018_TMP_OFFSET);
    this.fanSpeed = createFanSpeedRange2(EEP_A51018_FAN_OFFSET);
    this.occupancyEnabled = createOccupancyButtonEnableDisableBool(EEP_A51018_OED_OFFSET);
    this.occupancy = createOccupancyButtonBool(EEP_A51018_OB_OFFSET);

    this.sensorData = new EepData(
        EepType.EEP_TYPE_A51018, 4, illumination, setPoint,
