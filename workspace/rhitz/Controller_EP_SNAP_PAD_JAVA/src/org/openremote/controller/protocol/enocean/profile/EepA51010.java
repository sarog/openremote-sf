...
/**
 * Represents the EnOcean equipment profile (EEP) 'A5-10-10'. <p>
 *
 * <pre>
 *
 *     +------+------+---------------------------------------------------------------------------+
 ...
 *     +------+------+---------------------------------------------------------------------------+
 *     | TYPE |  10  |Temperature and Humidity Sensor, Set Point, Fan Speed and Occupancy Control|
 *     +------+------+---------------------------------------------------------------------------+
 *
 * </pre>
 *
 * The 'A5-10-10' profile data is transmitted by means of 4BS radio telegrams.
 ...
 *                                                                    | |   | |
 *            +-------------------------------------------------------+ +---+ +
 *            |   Set Point   |    Humidity   |  Temperature  |       | |   | |
 *            |      SP       |      HUM      |      TMP      |       | |   | |
 *            +-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+
 ...
 *     +-------------------------------------------------------------------------------------------------+
 *     |0     |8   |DB3.7..DB3.0|Set point     |  SP    |Set point(linear)   |   0..255  | 0..255   |N/A |
 *     +-------------------------------------------------------------------------------------------------+
 ...
 * @author Rainer Hitz
 */
public class EepA51010 extends EepA510XX
{
   ...
   */
  static final int EEP_A51010_SP_OFFSET = 0;

  /**
   * Bit offset of humidity data field.
   */
  static final int EEP_A51010_HUM_OFFSET = 8;

  /**
   * Bit offset of temperature data field.
   */
  static final int EEP_A51010_TMP_OFFSET = 16;

  /**
   * Bit offset of occupancy data field.
   */
  static final int EEP_A51010_OCC_OFFSET = 31;


  // Constructors ---------------------------------------------------------------------------------

  public EepA51010(DeviceID deviceID, String commandString) throws ConfigurationException
  {
    super(EepType.EEP_TYPE_A51010, deviceID);

    initCommand(commandString);

    this.setPoint = createSetPointRange8Bit(EEP_A51010_SP_OFFSET);
    this.humidity = createHumidityRange(EEP_A51010_HUM_OFFSET);
    this.temperature = createTemperatureRange8Bit2(EEP_A51010_TMP_OFFSET);
    this.occupancy = createOccupancyBool(EEP_A51010_OCC_OFFSET);

    this.sensorData = new EepData(EepType.EEP_TYPE_A51010, 4, setPoint, humidity, temperature, occupancy);
  }
