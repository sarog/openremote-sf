...
/**
 * Represents the EnOcean equipment profile (EEP) 'A5-10-1B'. <p>
 *
 * <pre>
 *
 *     +------+------+--------------------------------------------------------------------+
 ...
 *     +------+------+--------------------------------------------------------------------+
 *     | TYPE |  1B  |Supply voltage, Illumination, Temperature Sensor, Fan Speed         |
 ...
 *                                                                    | | | | |
 *            +-------------------------------------------------------+ +-+ + +
 *            |Supply Voltage |  Illumination |  Temperature  | |Fan  | | | | |
 *            |      SV       |      ILL      |      TMP      | |Speed| | | | |
 *            +-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+
 ...
 *     |      |    |            |              |        |error code          |           |          |    |
 *     +-------------------------------------------------------------------------------------------------+
 *     |8     |8   |DB2.7..DB2.0|Illumination  |  ILL   |Illumination(linear)|   0..250  | 0..1000  |lx  |
 *     |      |    |            |              |        |251: Over range,    |           |          |    |
 *     |      |    |            |              |        |252-255: reserved   |           |          |    |
 *     +-------------------------------------------------------------------------------------------------+
 ...
 * @author Rainer Hitz
 */
public class EepA5101B extends EepA510XX
{
   ...
   */
  static final int EEP_A5101B_SV_OFFSET = 0;

  /**
   * Bit offset of illumination data field.
   */
  static final int EEP_A5101B_ILL_OFFSET = 8;

  /**
   * Bit offset of temperature data field.
   */
  static final int EEP_A5101B_TMP_OFFSET = 16;

  /**
   * Bit offset of 'fan speed' data field.
   */
  static final int EEP_A5101B_FAN_OFFSET = 25;

  /**
   * Bit offset of 'occupancy enable/disable' data field.
   */
  static final int EEP_A5101B_OED_OFFSET = 30;

  /**
   * Bit offset of 'occupancy button' data field.
   */
  static final int EEP_A5101B_OB_OFFSET = 31;


  // Constructors ---------------------------------------------------------------------------------

  public EepA5101B(DeviceID deviceID, String commandString) throws ConfigurationException
  {
    super(EepType.EEP_TYPE_A5101B, deviceID);

    initCommand(commandString);

    this.supplyVoltage = createSupplyVoltageRange(EEP_A5101B_SV_OFFSET);
    this.illumination = createIlluminationRange(EEP_A5101B_ILL_OFFSET);
    this.temperature = createTemperatureRange8Bit3(EEP_A5101B_TMP_OFFSET);
    this.fanSpeed = createFanSpeedRange2(EEP_A5101B_FAN_OFFSET);
    this.occupancyEnabled = createOccupancyButtonEnableDisableBool(EEP_A5101B_OED_OFFSET);
    this.occupancy = createOccupancyButtonBool(EEP_A5101B_OB_OFFSET);

    this.sensorData = new EepData(
        EepType.EEP_TYPE_A5101B, 4, supplyVoltage, illumination,
