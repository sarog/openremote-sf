...
/**
 * Unit tests for {@link EepA50207} class.
 *
 * @author Rainer Hitz
 */
public class EepA50207Test extends EepA502XXTest
{
  ...
  @Test public void testBasicConstruction() throws Exception
  {
    ...
    Eep eep = EepType.lookup("A5-02-07").createEep(
        deviceID, Constants.TEMPERATURE_STATUS_COMMAND
    );

    Assert.assertTrue(eep instanceof EepA50207);
    Assert.assertEquals(EepType.EEP_TYPE_A50207, eep.getType());

    // Old EEP number ...

    eep = EepType.lookup("07-02-07").createEep(
        deviceID, Constants.TEMPERATURE_STATUS_COMMAND
    );
  ...
  {
    EepA50207 eep = (EepA50207)EepType.lookup("A5-02-07").createEep(
        deviceID, Constants.TEMPERATURE_STATUS_COMMAND
    );
