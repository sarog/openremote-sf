...
/**
 * Unit tests for {@link Esp2RPSTelegram} class.
 *
 * @author Rainer Hitz
 */
public class Esp2RPSTelegramTest
{
  ...
  {
    Esp2RPSTelegram telegram = new Esp2RPSTelegram(senderID, payload, status);
  ...
  @Test public void testSenderID() throws Exception
  {
    ...
    telegram = new Esp2RPSTelegram(senderID, payload, status);
  ...
  {
    Esp2RPSTelegram telegram = new Esp2RPSTelegram(null, payload, status);
  }
  ...
  private void assertTelegramAttributes(Esp2RPSTelegram telegram, Esp2PacketHeader.PacketType type,
                                        DeviceID senderID, byte payload, byte status)
  {
    Assert.assertEquals(type, telegram.getPacketType());
    Assert.assertEquals(AbstractEsp2RadioTelegram.RORG.RPS_ESP2, telegram.getRORG());
  ...
  private byte[] createDataGroup(DeviceID deviceID, byte payload, byte status)
  {
    ...
    dataGroupBytes[0] = AbstractEsp3RadioTelegram.RORG.RPS_ESP2.getValue();
