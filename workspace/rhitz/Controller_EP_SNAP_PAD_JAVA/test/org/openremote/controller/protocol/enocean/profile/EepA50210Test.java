...
/**
 * Unit tests for {@link EepA50210} class.
 *
 * @author Rainer Hitz
 */
public class EepA50210Test extends EepA502XXTest
{
  ...
  @Test public void testBasicConstruction() throws Exception
  {
    ...
    Eep eep = EepType.lookup("A5-02-10").createEep(
        deviceID, Constants.TEMPERATURE_STATUS_COMMAND
    );

    Assert.assertTrue(eep instanceof EepA50210);
    Assert.assertEquals(EepType.EEP_TYPE_A50210, eep.getType());

    // Old EEP number ...

    eep = EepType.lookup("07-02-10").createEep(
        deviceID, Constants.TEMPERATURE_STATUS_COMMAND
    );
  ...
  {
    EepA50210 eep = (EepA50210)EepType.lookup("A5-02-10").createEep(
        deviceID, Constants.TEMPERATURE_STATUS_COMMAND
    );
