...
public class EepA51202Test extends EepA512XXTest
{
  ...
  @Test public void testBasicConstruction() throws Exception
  {
    ...
    Eep eep = EepType.lookup("A5-12-02").createEep(
        deviceID, Constants.AMR_METER_READING_STATUS_COMMAND, null
    );

    Assert.assertTrue(eep instanceof EepA51202);
    Assert.assertEquals(EepType.EEP_TYPE_A51202, eep.getType());

    // Old EEP number ...

    eep = EepType.lookup("07-12-02").createEep(
        deviceID, Constants.AMR_METER_READING_STATUS_COMMAND, null
    );
  ...
  {
    EepA51202 eep = (EepA51202)EepType.lookup("A5-12-02").createEep(
        deviceID, Constants.AMR_METER_READING_STATUS_COMMAND, null
    );
