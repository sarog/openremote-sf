...
/**
 * Controls all the screen views in a group. The main operation of application
 * is handled in it.
 * 
 * @author Tomsky Wang
 * @author Dan Cong
...
public class GroupActivity extends GenericActivity implements
		OnGestureListener, ORConnectionDelegate {
	...
	/**
	 * The layout is the activity's content view, it contains
	 * currentScreenViewFlipper.
	 */
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		if (intent.getStringExtra("NFCValue") != null || intent.getStringExtra("SCAN_RESULT") != null) {
			
			showObjectScreen(intent, "nfc");
	...
	/**
	 * Inits a orientation listener, set request orientation be sensor when the
	 * current screen's orientation equals device orientation.
	 */
			...
			public void onOrientationChanged(int orientation) {
				...
				if (orientation > 315 || orientation < 45
					...
					if (getRequestedOrientation() == ActivityInfo.SCREEN_ORIENTATION_PORTRAIT
							...
							&& currentScreen.getInverseScreenId() > 0) {
						setRequestedOrientation(ActivityInfo.SCREEN_ORIENTATION_SENSOR);
					} else if (!currentScreen.isLandscape()
							&& currentScreen.getInverseScreenId() == 0) {
	...
	private void addControllerRefreshEventListener() {
		...
		ORListenerManager.getInstance().addOREventListener(
				ListenerConstant.FINISH_GROUP_ACTIVITY, new OREventListener() {
					@Override
	...
	private int getCurrentGroupId() {
		return currentGroupView == null ? 0 : currentGroupView.getGroup()
				.getGroupId();
	}
	...
	private int getScreenIndex(int screenId, boolean landscape) {
		if (currentGroupView != null && currentGroupView.getGroup() != null) {
			return currentGroupView.getGroup().getScreenIndexByOrientation(
					XMLEntityDataBase.getScreen(screenId), landscape);
		}
		return -1;
	}

	/**
	 * Init group and screen, if it is first access, init the first group and
	 * first screen, else init the last time group and screen. If no group or
	 * screen found, can configure settings.
	 * 
	 */
	private void recoverLastGroupScreen() {
		...
		if (lastGroup == null || lastGroup.getScreens().size() == 0) {
			if (!useLocalCache) {
				ViewHelper.showAlertViewWithSetting(this, "No Group Found",
						"please config Settings again");
			}
			return;
		}
		...
		if (screenSize == 0) {
			ViewHelper.showAlertViewWithTitle(this, "Info", "The group "
					+ lastGroup.getName() + " has no "
					+ (isLandscape ? "landscape" : "portrait") + " screen.");
		...
		currentScreenViewFlipper = currentGroupView
				.getScreenViewFlipperByOrientation(isLandscape);

		int lastScreenID = UserCache.getLastScreenId(this);
		if (lastScreenID > 0
				&& lastGroup.canfindScreenByIdAndOrientation(lastScreenID,
						isLandscape)) {
			currentScreenViewFlipper.setDisplayedChild(getScreenIndex(
					lastScreenID, isLandscape));
		}
		contentLayout = new LinearLayout(this);
		contentLayout.addView(currentScreenViewFlipper);
		this.setContentView(contentLayout);
	
		ScreenView currentScreenView = (ScreenView) currentScreenViewFlipper
				.getCurrentView();
		if (currentScreenView == null) {
			return;
		}
		UserCache.saveLastGroupIdAndScreenId(GroupActivity.this,
				currentGroupView.getGroup().getGroupId(), currentScreenView
	...
	private void addNaviagateListener() {
		ORListenerManager.getInstance().addOREventListener(
				ListenerConstant.ListenerNavigateTo, new OREventListener() {
					public void handleEvent(OREvent event) {
						Log.i("view", "handleEvent" );
	...
	private boolean moveRight() {
		...
		if (currentScreenViewFlipper.getDisplayedChild() < screenSize - 1) {
			...
			startCurrentPolling();
			UserCache.saveLastGroupIdAndScreenId(GroupActivity.this,
					currentGroupView.getGroup().getGroupId(),
					((ScreenView) currentScreenViewFlipper.getCurrentView())
							.getScreen().getScreenId());
			return true;
		}
		return false;
	...
	/**
	 * Detect the gesture and handle it. Support fling type: "right to left",
	...
	/**
	 * Handle screen gesture by type. Support navigate and send command.
	 * 
	 * @param gestureType
	 *            the gesture type
	 */
	private void onScreenGestureEvent(int gestureType) {
		...
		currentScreen = ((ScreenView) currentScreenViewFlipper.getCurrentView())
				.getScreen();
		Gesture gesture = currentScreen.getGestureByType(gestureType);
		if (gesture != null) {
			if (gesture.isHasControlCommand()) {
				new ORRoundRobinConnection(this, ORHttpMethod.POST, true,
						AppSettingsModel.getSecuredServer(this)
								+ "/rest/control/" + gesture.getComponentId()
								+ "/swipe", this);
			}
	...
	/**
	 * Handle default menu("readqrcode", "readnfc", "setting", "logout").
	 * 
	 * @param item
	 *            the item
	 */
	public void handleMenu(MenuItem item) {
		...
		case Constants.MENU_ITEM_LOGOUT:
			doLogout();
			break;
		case Constants.MENU_ITEM_READQRCODE:
			Intent intent1 = new Intent("com.google.zxing.client.android.SCAN");
			intent1 = intent1.putExtra("SCAN_MODE", "QR_CODE_MODE");
		    startActivityForResult(intent1, 0);			
			break;

		case Constants.MENU_ITEM_READNFC:
			Intent intent2 = new Intent();
			intent2.setClass(GroupActivity.this, NfcActivity.class);
	...
	/**
	 * For read QR-Codes Tag's and show the screen of the corresponding object
	 */
	public void onActivityResult(int requestCode, int resultCode, Intent intent) {
		if (requestCode == 0) {
			if (resultCode == RESULT_OK) {
	
				showObjectScreen(intent,"qrcode");
			}	
		}		
	}

	
	public void showObjectScreen(Intent intent, String TagType) {
		...
		{
			contents = intent.getStringExtra("NFCValue"); 
		}
		
		// Screens is a HashMap which contains "id" of all the screens

		if (XMLEntityDataBase.screens.containsKey(Integer.parseInt(contents)) == true) {
				// Find the right one and open it
			if (currentGroupView
					.getGroup()
					.getScreens()
					.contains(
							XMLEntityDataBase.screens.get(Integer
									.parseInt(contents))) == false) {
				
				UserCache
						.saveLastGroupIdAndScreenId(this, XMLEntityDataBase
								.getScreen(Integer.parseInt(contents)).group
								.getGroupId(), Integer.parseInt(contents));
			
			}

			else {
				
				UserCache.saveLastGroupIdAndScreenId(this, currentGroupView
						.getGroup().getGroupId(), Integer.parseInt(contents));
				
				
			}
			// We use saveLastGroupIdAndScreenId for make that lastScreen takes id of the screen
			// that we want to open

			this.recoverLastGroupScreen();
		}
		
		else{
			 
	        LayoutInflater factory = LayoutInflater.from(this);
	        final View alertDialogView = factory.inflate(R.layout.tagundiscovered_view, null);
	 
	        AlertDialog.Builder adb = new AlertDialog.Builder(this);
	 
	        adb.setView(alertDialogView);
	 
	        adb.setTitle("Sorry, Tag Undiscovered");
	...
	/**
	 * If there is no global and local tabbar, create default menu. Contains
	 * "setting" and "logout".
	 * 
	...
	public boolean onCreateOptionsMenu(Menu menu) {
		...
		MenuItem qrcode = menu.add(-1, Constants.MENU_ITEM_READQRCODE,0,
				"QR-Code Reading");
		qrcode.setIcon(R.drawable.ic_qrcode);
		
		MenuItem nfc = menu.add(-1, Constants.MENU_ITEM_READNFC, 1, "NFC Reading");
		nfc.setIcon(R.drawable.ic_nfc);
		
		MenuItem setting = menu.add(-1, Constants.MENU_ITEM_SETTING,2,
				R.string.setting);
		setting.setIcon(R.drawable.ic_menu_manage);
		
		MenuItem logout = menu.add(-1, Constants.MENU_ITEM_LOGOUT, 3,
				R.string.logout);
	...
	/**
	 * If there have global or local tabbar, create and update menu before the
	 * menu is shown.
	 * 
			...
			for (int i = 0; i < itemSize; i++) {
				...
				if (items.get(i).getImage() != null) {
					menuItem.setIcon(ImageUtil
							.createFromPathQuietly(Constants.FILE_FOLDER_PATH
	...
	/**
	 * When the activity is stoped, unregister sensor listener, stop current
	 * screen's polling.
	 * 
	...
	protected void onResume() {
		...
		if (getIntent().getDataString() != null) {
			useLocalCache = true;
			ViewHelper.showAlertViewWithSetting(this, "Using cached content",
	...
	private boolean navigateTo(Navigate navigate) {
		if (navigate.isNextScreen()) {
			...
			return navigateToGroup(navigate.getToGroup(),
					navigate.getToScreen());
		} else if (navigate.isBack()) {
			if (navigationHistory.size() > 0) {
				Navigate backward = navigationHistory.get(navigationHistory
						.size() - 1);
				if (backward.getFromGroup() > 0 && backward.getFromScreen() > 0) {
					if (backward.getFromGroup() == getCurrentGroupId()) {
						isNavigetionBackward = getScreenIndex(
								backward.getFromScreen(), isLandscape) < getScreenIndex(
								currentScreen.getScreenId(), isLandscape);
					}
					navigateToGroup(backward.getFromGroup(),
							backward.getFromScreen());
				}
	...
	private void doLogout() {
		...
		if (!TextUtils.isEmpty(password)) {
			...
			ViewHelper.showAlertViewWithTitle(GroupActivity.this, "Logout",
	...
	private boolean navigateToGroup(int toGroupId, int toScreenId) {
		...
		if (targetGroup != null) {
			...
			if (currentGroupView.getGroup().getGroupId() != toGroupId) {
				...
				if (toScreenId > 0) {
					newOrientation = XMLEntityDataBase.getScreen(toScreenId)
							.isLandscape();
				}
				...
				currentScreenViewFlipper = targetGroupView
						.getScreenViewFlipperByOrientation(newOrientation);
				currentGroupView = targetGroupView;
				contentLayout.addView(currentScreenViewFlipper);
				if (toScreenId > 0
						&& targetGroup.canfindScreenByIdAndOrientation(
								toScreenId, newOrientation)) {
					currentScreenViewFlipper.setDisplayedChild(getScreenIndex(
							toScreenId, newOrientation));
				}
				screenSize = targetGroup
						.getScreenSizeByOrientation(newOrientation);
				...
				// in same group.
				boolean newOrientation = XMLEntityDataBase
						.getScreen(toScreenId).isLandscape();
				if (newOrientation != currentOrientation) {
					screenSize = targetGroup
							.getScreenSizeByOrientation(newOrientation);
					contentLayout.removeView(currentScreenViewFlipper);
			...
			startCurrentPolling();
			currentScreen = ((ScreenView) currentScreenViewFlipper
					.getCurrentView()).getScreen();
			return true;
		}
		return false;
	}
	...
	/**
	 * Handle navigate, if navigate to group or to screen success, store the
	...
	private void handleNavigate(Navigate navigate) {
		Log.i("view", "handleNavigate" );
		Navigate historyNavigate = new Navigate();
		if (currentGroupView.getGroup() != null) {
			historyNavigate.setFromGroup(currentGroupView.getGroup()
					.getGroupId());
			if (currentScreenViewFlipper == null) {
				return;
			}
			ScreenView sv = (ScreenView) currentScreenViewFlipper
					.getCurrentView();
			if (sv == null) {
				return;
			} else {
			...
			if (navigateTo(navigate)) {
				UserCache.saveLastGroupIdAndScreenId(GroupActivity.this,
						currentGroupView.getGroup().getGroupId(),
						((ScreenView) currentGroupView
								.getScreenViewFlipperByOrientation(isLandscape)
								.getCurrentView()).getScreen().getScreenId());
	...
	public void urlConnectionDidReceiveResponse(HttpResponse httpResponse) {
		...
		if (responseCode != 200) {
			if (responseCode == 401) {
				...
				ViewHelper.showAlertViewWithTitle(this, "Send Request Error",
						ControllerException
	...
	private void rotateToIntefaceOrientation() {
		...
		if (currentGroupView != null) {
			...
			contentLayout.addView(currentScreenViewFlipper);
			currentScreenViewFlipper.setDisplayedChild(getScreenIndex(
					inverseScreenId, isLandscape));
			startCurrentPolling();
			if (currentGroupView.getGroup() != null) {
				screenSize = currentGroupView.getGroup()
						.getScreenSizeByOrientation(isLandscape);
				UserCache
						.saveLastGroupIdAndScreenId(GroupActivity.this,
								currentGroupView.getGroup().getGroupId(),
								((ScreenView) currentScreenViewFlipper
										.getCurrentView()).getScreen()
										.getScreenId());
			}
		}
