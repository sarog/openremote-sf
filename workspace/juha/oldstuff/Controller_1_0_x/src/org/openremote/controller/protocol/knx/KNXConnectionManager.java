...
import static org.openremote.controller.protocol.knx.KNXConnectionManager.SystemProperty.KNX_CLIENT_ADDRESS;
import static org.openremote.controller.protocol.knx.KNXConnectionManager.SystemProperty.KNX_GATEWAY_ADDRESS;
import static org.openremote.controller.protocol.knx.KNXConnectionManager.SystemProperty.KNX_CLIENT_PORT;
...
class KNXConnectionManager
{
  ...
  // TODO :
  //   all the settings below should be externalized,
  //   ephemeral (zero) port should not be the default

  public final static int DEFAULT_CLIENT_DISCOVERY_PORT  = 11223;  // zero = any free port
  private final static int CLIENT_CONNECTION_PORT         = 11224;  // zero = any free port
   ...
   * {@value SystemProperty#KNX_CLIENT_ADDRESS_PROPERTY} system property can be used to set
   * this value.  <p>
   *
  ...
  protected void start() throws ConnectionException
  {
    ...
    try
    {
      // This will return candidate IP addresses to attempt KNX discovery on, or only one IP
      // address if KNX_LOCAL_BIND_ADDRESS has been set...

      Set<InetAddress> nics = resolveLocalAddresses();

      // Start discovery unless KNX_GATEWAY_ADDRESS has been configured...

      if (!KNX_GATEWAY_ADDRESS.exists())
      {
        for (InetAddress clientAddress : nics)
        {
          Discoverer discoverer = new Discoverer(clientAddress, DEFAULT_CLIENT_DISCOVERY_PORT, DISCOVERY_USE_NAT);

          try
          {
            discoverer.startSearch(
                DEFAULT_CLIENT_DISCOVERY_PORT,
   ...
   * {@value SystemProperty#KNX_CLIENT_ADDRESS_PROPERTY} property.
   * The {@link SystemProperty#KNX_CLIENT_ADDRESS} property should have as its value a valid IPv4
   * address in the machine hosting this KNX connection manager.  <p>
   *
   ...
   *                             machine or if {@link SystemProperty#KNX_CLIENT_ADDRESS} was
   *                             configured but could not be resolved to a valid address and
   *                             connected to
   *
  ...
  protected Set<InetAddress> resolveLocalAddresses() throws ConnectionException
  {
    ...
    if (KNX_CLIENT_ADDRESS.exists())
    {
      InetAddress clientDiscoveryAddress = resolveClientDiscoveryAddress();

      if (clientDiscoveryAddress != null)
      {
        log.info(
            "Found '" + KNX_CLIENT_ADDRESS + "' system property and resolved it to: " +
             clientDiscoveryAddress
        );

        Set<InetAddress> set = new HashSet<InetAddress>(1);
        set.add(clientDiscoveryAddress);
      ...
      else
      {
        throw new ConnectionException(
            "Property '" + KNX_CLIENT_ADDRESS + "' was configured but could not be resolved " +
...
//  /**
//   * A quick helper method to detect if the {@value SystemProperty#KNX_CLIENT_ADDRESS_PROPERTY}
//   * system property has been set.
//   *
//   * @return true if {@value SystemProperty#KNX_CLIENT_ADDRESS_PROPERTY} has been set;
//   *         false otherwise
//   */
//  private boolean hasExplicitAddressBinding()
//  {
//    return KNX_CLIENT_ADDRESS.getValue() != null;
//  }
//
//
//  /**
//   * A quick helper method to detect if the {@value SystemProperty#KNX_GATEWAY_ADDRESS_PROPERTY}
//   * system property has been set.
//   *
//   * @return true if {@value SystemProperty#KNX_GATEWAY_ADDRESS_PROPERTY} has been set;
//   *         false otherwise
//   */
//  private boolean hasExplicitKNXGatewayAddress()
//  {
//    return KNX_GATEWAY_ADDRESS.getValue() != null;
//  }
//
//
//  /**
//   * Utility method to retrieve the value of {@link SystemProperty#KNX_CLIENT_ADDRESS}
//   * system property.  <p>
//   *
...
//   *
//   * @return the value of {@link SystemProperty#KNX_CLIENT_ADDRESS} if set, or null
//   */
...
//
//  /**
//   * Utility method to retrieve the value of {@link SystemProperty#KNX_GATEWAY_ADDRESS}
//   * system property.  <p>
//   *
//   * Executed in a privileged code block, so as long as the calling code has sufficient permissions
//   * we don't require any extra privileges.
//   *
//   * @return the value of {@link SystemProperty#KNX_GATEWAY_ADDRESS} if set, or null
//   */
//  private String getKNXGatewayAddressValue()
//  {
//    try
//    {
//      // START PRIVILEGED CODE BLOCK --------------------------------------------------------------
...
//            public String run()
//            {
//              return System.getProperty(KNX_GATEWAY_ADDRESS);
//            }
//          }
//      );
//      // END PRIVILEGED CODE BLOCK ----------------------------------------------------------------
...
//    catch (SecurityException exception)
//    {
//      log.error("Security manager has denied access to '" + KNX_GATEWAY_ADDRESS +
//                "' propery (" + exception.getMessage() + ").", exception);
//
//      // Can't get the property, so act as if it was not set...
//
...
//  }
  ...
  /**
   * Attempts to resolve a configured {@link SystemProperty#KNX_CLIENT_ADDRESS} system property
   * into a valid InetAddress. <p>
   *
   * @return  an <code>InetAddress</code> resolved from {@link SystemProperty#KNX_CLIENT_ADDRESS}
   *          system property or <code>null</code> if the property was not set or could not be
   *          resolved or connected to.
   */
  private InetAddress resolveClientDiscoveryAddress()
  {
    final String clientAddress = KNX_CLIENT_ADDRESS.getValue();
    ...
    try
    {
      ...
      return AccessController.doPrivileged(
          new PrivilegedExceptionAction<InetAddress>()
          {
            ...
            {
              return InetAddress.getByName(clientAddress);
            }
          }
      );
    ...
    {
      log.error("Security manager denied access to address '" + clientAddress + "': " +
    ...
    {
      log.warn(
          "Unable to resolve " + KNX_CLIENT_ADDRESS + " property with value '" +
          clientAddress + "': (" + exception.getException().getMessage() + ")", exception
      );
  ...
  /**
   * Attempts to resolve a configured {@link #KNX_GATEWAY_ADDRESS} system property into a valid
   * InetAddress
   *
   * @return  an <code>InetAddress</code> resolved from {@link #KNX_GATEWAY_ADDRESS} system
   *          property or <code>null</code> if the property was not set or could not be resolved
   *          or connected to.
   */
  private InetAddress getConfiguredKNXGateway()
  {
    final String gatewayAddress = getKNXGatewayAddressValue();
    ...
    else
    {
      ...
      {
        log.error("Security manager denied access to address '" + gatewayAddress + "': " +
                  exception.getMessage(), exception);
      ...
      {
        log.error("Could not resolve KNX gateway address '" + gatewayAddress +
                  "': " + exception.getMessage(), exception);
   ...
   * and point-to-point (e.g. PPP to modem) interfaces.  <p>
   *
   * NOTE :
  ...
  private synchronized HPAI waitForDiscovery() throws ConnectionException
  {
    ...
    {
      if (hasExplicitKNXGatewayAddress())
      {
        clientIP = address;
        HPAI gateway = new HPAI(HPAI.IPV4_UDP, getConfiguredKNXGateway().);
      }
  ...
  // Nested Classes, Enums and Interfaces ---------------------------------------------------------


  /**
   * TODO :
   *
   *   these belong to a wider scope into a utility package with all other system properties
   *   as well
   *
   */
  ...
  {
    /**
     * A system property "{@value #KNX_GATEWAY_ADDRESS_PROPERTY}" that can be set to a
     * specific IPv4 address to bypass the KNX gateway discovery and connect directly to a KNX
     * gateway. This is especially useful when the KNX gateway is located in another subnet from
     * the controller and IP multicast may not be enabled or configured to propagate across
     * switches.
     */
    KNX_GATEWAY_ADDRESS(SystemProperty.KNX_GATEWAY_ADDRESS_PROPERTY),

    /**
     * TODO
     */
    KNX_GATEWAY_PORT(SystemProperty.KNX_GATEWAY_PORT_PROPERTY),

    /**
     * A system property "{@value #KNX_CLIENT_ADDRESS_PROPERTY}" that can be set to a
     * specific IPv4 address on the system to force KNX related IP communication to a specific
     * client network interface. <p>
     *
    ...
    KNX_CLIENT_ADDRESS(SystemProperty.KNX_CLIENT_ADDRESS_PROPERTY),

    /**
     * TODO
     */
    KNX_CLIENT_PORT(SystemProperty.KNX_CLIENT_PORT_PROPERTY);


    // Constants ----------------------------------------------------------------------------------

    public final static String KNX_CLIENT_ADDRESS_PROPERTY     = "knx.client.ip";
    public final static String KNX_CLIENT_PORT_PROPERTY        = "knx.client.port";
    public final static String KNX_GATEWAY_ADDRESS_PROPERTY    = "knx.gateway.ip";
    public final static String KNX_GATEWAY_PORT_PROPERTY       = "knx.gateway.port";
    ...
    private SystemProperty(String propertyName)
    {
    ...
    /**
     * Returns a system property value or <code>null</code> if the system property has not been
     * set. <p>
     *
     * System property access is executed in a privileged block so only the calling codebase
     * needs to have appropriate permissions if security manager has been enabled.
     *
     * @see #getValue(String)
     *
     * @return system property value corresponding to this enum or <code>null</code> if the
    ...
    /**
     * Returns a system property value for one of the elements in this enum set. The value string
     * must be parseable to an integer type. If the system property has not been set for the
     * runtime, a default value is returned. <p>
     *
     * System property access is executed in a privileged block so only the calling codebase
     * needs to have appropriate permissions if security manager has been enabled.
     *
    ...
    public final int getValue(int defaultValue)
    {
      ...
      try
      {
        value = this.getValue(Integer.toString(defaultValue));
      ...
      {
        log.warn(
            "The system property '" + this.getPropertyName() + "' was configured with a " +
            "non-integer value: '" + value + "'. Using default value " + defaultValue + " instead."
        );
    ...
    /**
     * Returns a system property value for one of the elements in this enum set. If the system
     * property has not been set for the runtime, a default value is returned. <p>
     *
     * System property access is executed in a privileged block so only the calling codebase
     * needs to have appropriate permissions if security manager has been enabled.
     *
     * @see #getValue
     *
     * @param defaultValue  default value to return if the system property has not been set for
     *                      the runtime
     *
     * @return system property value corresponding to this enum or default value if the
     *         property has not been set
     */
    public final String getValue(final String defaultValue)
    {
      ...
      {
        /*
         * Only allow access to specific system properties defined in this enum set. Should under
         * no circumstances allow privileged access to arbitrary system properties.
         */
      ...
      {
        // In case security manager has been set and calling code lacks the required permissions,
        // report and error and return the default value if any...

        log.error(
            "Security manager has denied access to '" + getPropertyName() + "' propery (" +
             exception.getMessage() + "). Returning default value of '" + defaultValue + "'.",
             exception
        );
    ...
    /**
     * Returns the name of this system property, as defined by {@link #getPropertyName()}.
     *
     * @return this system property name
     */
