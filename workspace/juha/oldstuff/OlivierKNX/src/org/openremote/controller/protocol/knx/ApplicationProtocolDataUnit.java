...
/**
 * This class represents application protocol data unit (APDU) in KNX specification.
 * <p>
 * 
 ...
 * </pre>
 * 
 * For data that is 6 bits or less in length, only the first two bytes are used in a Common EMI frame. Common EMI frame
 * also carries the information of the expected length of the Protocol Data Unit (PDU). Data payload can be at most 14
 * bytes long.
 * <p>
 * 
 * The first byte is a combination of transport layer control information (TPCI) and application layer control
 * information (APCI). First 6 bits are dedicated for TPCI while the two least significant bits of first byte hold the
 * two most significant bits of APCI field, as follows:
 * 
 * <pre>
 * {@code
 * 
 ...
 * {@link org.openremote.controller.protocol.knx.ApplicationLayer.Service application layer service} is being used. The
 * second byte bit structure is as follows:
 * 
 * <pre>
 * {@code
 * 
...
class ApplicationProtocolDataUnit {
   ...
   /**
    * Represents the full APDU (APCI + data) for Group Value Write service request with DPT 1.001 (Switch) to state
    * 'ON'.
    * 
   ...
   final static ApplicationProtocolDataUnit WRITE_SWITCH_ON = new ApplicationProtocolDataUnit(
         ApplicationLayer.Service.GROUPVALUE_WRITE_6BIT, Bool.ON);
   ...
   final static ApplicationProtocolDataUnit WRITE_SWITCH_OFF = new ApplicationProtocolDataUnit(
         ApplicationLayer.Service.GROUPVALUE_WRITE_6BIT, Bool.OFF);

   /**
    * Represents the full APDU (APCI bits) for Group Value Read request for data points with DPT 1.001 (Switch) type.
    * 
   ...
   final static ApplicationProtocolDataUnit READ_SWITCH_STATE = new ApplicationProtocolDataUnit(
         ApplicationLayer.Service.GROUPVALUE_READ, Bool.READ_SWITCH);
   ...
   /**
    * Determines from the Application Protocol Control Information (APCI) bits in an application protocol data unit
    * (APDU) bytes whether the application level service corresponds to Group Value Response service.
    * 
    * @see org.openremote.controller.protocol.knx.ApplicationLayer.Service#GROUPVALUE_RESPONSE_6BIT
    * 
    * @param apdu
    *           Byte array containing the application protocol data unit payload. Only the first two bytes are
    *           inspected. This parameter can therefore contain only a partial APDU with only the first two bytes of
    *           APCI information or the entire APDU with data included.
    * 
   ...
   static boolean isGroupValueWriteReq(byte[] apdu) {
      ...
      //
      // Byte 1 : bits xxxxxx00 - first six bits are part of TPCI
      // Byte 2 : bits 10xxxxxx - last six bits are either data (6 bit return values)
      // or zero for larger than 6 bit return values
      return ((apdu[0] & 0x3) == 0x00 && (apdu[1] & 0xC0) == 0x80);
   }

   /**
    * Constructs an APDU corresponding to a Group Value Write service for a device expecting a 3-bit dim control data
    * point type (DPT 3.007).
    * <p>
    * 
    ...
    * {@link org.openremote.controller.protocol.knx.datatype.Bool#CALCULATED}, respectively).
    * <p>
    * 
    ...
    * 
    * @param controlValue
    *           must be one of DataType.Boolean.INCREASE, DataType.Boolean.DECREASE, DataType.Boolean.UP,
    *           DataType.Boolean.DOWN, DataType.Boolean.FIXED or DataType.Boolean.CALCULATED
    * 
    * @param dimValue
    *           must be in range of [0-7]
    * 
    * @return APDU instance for a 3-bit dim control Group Value Write service with a given control bit and range bits
    */
   static ApplicationProtocolDataUnit create3BitDimControl(Bool controlValue, int dimValue) {
      return new ApplicationProtocolDataUnit(ApplicationLayer.Service.GROUPVALUE_WRITE_6BIT, new Controlled3Bit(
            DataPointType.Control3BitDataPointType.CONTROL_DIMMING, controlValue, dimValue));
   }

   /**
    * Constructs an APDU corresponding to a Group Value Write service for a device expecting an 8-bit unsigned scaling
    * value (DPT 5.001).
    * <p>
    * 
    ...
    * 
    * @param parameter
    *           scaling value for percentage range
    * 
    * @return APDU instance for a 8-bit unsigned scaling value
    * 
    * @throws ConversionException
    *            if the value is not in a given range or cannot be scaled to the desired range
    */
   static ApplicationProtocolDataUnit createScaling(CommandParameter parameter) throws ConversionException {
      int value = parameter.getValue();

      if (value < 0 || value > 100) {
         throw new ConversionException("Expected value is in range [0-100] (percentage), received " + value);
      }
      ...
      return new ApplicationProtocolDataUnit(ApplicationLayer.Service.GROUPVALUE_WRITE, new Unsigned8Bit(
            DataPointType.Unsigned8BitValue.SCALING, value));
   }
    ...
    * 
    * @param service
    *           application layer service as defined in
    ...
    * 
    * @return application layer service name associated with this APDU and the contents of this APDU as unsigned hex
    *         values.
    */
   @Override
   ...
   /**
    * Returns the actual data payload without application protocol control information (APCI) bits as a string value.
    * The bytes in the payload are formatted as unsigned hex strings. An example output could look like:
    * 
    * <pre>
    * {@code
    * 
   ...
   /**
    * Returns the data length of the data type associated with this APDU. See {@link DataType#getDataLength()} for
    * details.
    * 
    * @see DataType#getDataLength()
    * 
    * @return returns the APDU data payload length in bytes, as specified in {@link DataType#getDataLength()}
    */
   int getDataLength() {
   ...
   /**
    * Attempts to convert the data payload of this APDU into a KNX Boolean datatype value.
    * <p>
    * 
    ...
    * 
    * @return {@link org.openremote.controller.protocol.knx.datatype.Bool#TRUE} for value 1,
    *         {@link org.openremote.controller.protocol.knx.datatype.Bool#FALSE} for value 0.
    * 
    * @throws ConversionException
    *            if the data payload cannot be converted to KNX Boolean type.
    */
   Bool convertToBooleanDataType() throws ConversionException {
      ...
      if (data.length == 1) {
         ...
         throw new ConversionException("Was expecting 6-bit boolean datatype in payload but data was " + data.length
               + " bytes long.");
      }
   }

   /**
    * Returns the application protocol data unit (APDU) including the Control Information (ACPI) bits and data value.
    * <p>
    * 
    * Returned byte array is at minimum 2 bytes long (for 6-bit data values), and maximum of 16 bytes long (with a
    * largest possible 14 byte data value).
    * <p>
    * 
    * The six most significant bits of the first byte in the array are Transport Protocol Control Information (TCPI)
    * bits which are all set to zero.
    * 
   ...
   static class ResponseAPDU extends ApplicationProtocolDataUnit {
      ...
      static ResponseAPDU create6BitResponse(final byte[] apdu) {
         return new ResponseAPDU(ApplicationLayer.Service.GROUPVALUE_RESPONSE_6BIT, 1 /* data length */,
      ...
      static ResponseAPDU create8BitResponse(final byte[] apdu) {
         return new ResponseAPDU(ApplicationLayer.Service.GROUPVALUE_RESPONSE, 2, /* Data length */
         new byte[] { apdu[2] });
      }
      ...
      static ResponseAPDU createTwoByteResponse(final byte[] apdu) {
         return new ResponseAPDU(ApplicationLayer.Service.GROUPVALUE_RESPONSE, 3, /* Data length */
      ...
      static ResponseAPDU createThreeByteResponse(final byte[] apdu) {
         return new ResponseAPDU(ApplicationLayer.Service.GROUPVALUE_RESPONSE, 4, /* Data length */
      ...
      static ResponseAPDU createFourByteResponse(final byte[] apdu) {
         return new ResponseAPDU(ApplicationLayer.Service.GROUPVALUE_RESPONSE, 5, /* Data length */
      ...
      static ResponseAPDU createStringResponse(final byte[] apdu) {
         ...
         return new ResponseAPDU(ApplicationLayer.Service.GROUPVALUE_RESPONSE, stringData.length + 1, /* Data length */
         stringData);
      }
      ...
      ApplicationProtocolDataUnit resolve(DataPointType dpt) {
         if (dpt instanceof DataPointType.BooleanDataPointType) {
            ...
            return new ApplicationProtocolDataUnit(getApplicationLayerService(), resolveToBoolean(bool, getDataType()
                  .getData()[0]));
         }
            ...
            return new ApplicationProtocolDataUnit(getApplicationLayerService(), resolveTo8BitValue(value,
                  getDataType().getData()[0]));
         }
      ...
      private Bool resolveToBoolean(DataPointType.BooleanDataPointType dpt, int value) {
         if (dpt == DataPointType.BooleanDataPointType.SWITCH) return (value == 0) ? Bool.OFF : Bool.ON;

         else if (dpt == DataPointType.BooleanDataPointType.BOOL) return (value == 0) ? Bool.FALSE : Bool.TRUE;

         else if (dpt == DataPointType.BooleanDataPointType.ENABLE) return (value == 0) ? Bool.DISABLE : Bool.ENABLE;

         else if (dpt == DataPointType.BooleanDataPointType.RAMP) return (value == 0) ? Bool.NO_RAMP : Bool.RAMP;

         else if (dpt == DataPointType.BooleanDataPointType.ALARM) return (value == 0) ? Bool.NO_ALARM : Bool.ALARM;

         else if (dpt == DataPointType.BooleanDataPointType.BINARY_VALUE) return (value == 0) ? Bool.LOW : Bool.HIGH;

         else if (dpt == DataPointType.BooleanDataPointType.STEP) return (value == 0) ? Bool.DECREASE : Bool.INCREASE;

         else if (dpt == DataPointType.BooleanDataPointType.UP_DOWN) return (value == 0) ? Bool.UP : Bool.DOWN;

         else if (dpt == DataPointType.BooleanDataPointType.OPEN_CLOSE) return (value == 0) ? Bool.OPEN : Bool.CLOSE;

         else if (dpt == DataPointType.BooleanDataPointType.START) return (value == 0) ? Bool.STOP : Bool.START;

         else if (dpt == DataPointType.BooleanDataPointType.STATE) return (value == 0) ? Bool.INACTIVE : Bool.ACTIVE;

         else if (dpt == DataPointType.BooleanDataPointType.INVERT) return (value == 0) ? Bool.NOT_INVERTED
               : Bool.INVERTED;

         else if (dpt == DataPointType.BooleanDataPointType.DIM_SEND_STYLE) return (value == 0) ? Bool.START_STOP
               : Bool.CYCLICALLY;

         else if (dpt == DataPointType.BooleanDataPointType.INPUT_SOURCE) return (value == 0) ? Bool.FIXED
               : Bool.CALCULATED;
