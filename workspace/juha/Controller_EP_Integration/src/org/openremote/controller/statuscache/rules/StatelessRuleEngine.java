 ...
 */
public class StatelessRuleEngine extends EventProcessor
{
  ...
  private final static Logger log = Logger.getLogger(Constants.RUNTIME_EVENTPROCESSOR_LOG_CATEGORY + ".drools");

  private final static Logger initLog = Logger.getLogger(Constants.EVENT_PROCESSOR_INIT_LOG_CATEGORY);
  ...
  @Override public Event push(Event event)
  {
    ...
    // evaluate this event against currently deployed rules...

    StatelessKnowledgeSession session = kb.newStatelessKnowledgeSession();

    //session.setGlobal("sensor", SensorFacade());
   ...
   *
   */
  @Override public void start() throws InitializationException
  {
