...
public class SensorBuilder extends ObjectBuilder
{
   ...
   *
   * @param deployer  deployer instance this sensor builder will register with
   * @param cache     deployer's associated device state cache -- the created sensors will use
...
//   */
//  private Sensor build(Element sensorElement) throws InitializationException
//  {
//    String sensorIDValue = sensorElement.getAttributeValue("id");
//    String sensorName = sensorElement.getAttributeValue("name");
//
...
//  }
  ...
  /**
   *
   * TODO : remove the enumsensortype use
   */
  private EnumSensorType parseSensorType(Element sensorElement) throws XMLParsingException
  {
    ...
    try
    {
      return EnumSensorType.valueOf(typeValue.toUpperCase());
    }
...
//  }
  ...
  /**
   * TODO
   *   - parsing other content is ignored.
   *
   * @param sensorElement
   * @return
   * @throws InitializationException
      ...
      if (sensorProperty.getName().equalsIgnoreCase(XMLMapping.XML_INCLUDE_ELEMENT_NAME))
      {
        ...
        if (includeTypeAttrValue.equalsIgnoreCase(XMLMapping.XML_INCLUDE_ELEMENT_TYPE_COMMAND))
        {
          ...
          try
          {
            ...
            Command eventProducer = protocolHandlerFactory.getCommand(eventProducerElement); // TODO : should go through deployer to ensure we only do it once
