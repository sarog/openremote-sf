 ...
 */
package org.openremote.controller.protocol.openwebnet.connector;
...
import org.openremote.controller.protocol.openwebnet.datastructure.action.Action;
import org.openremote.controller.protocol.openwebnet.datastructure.command.CommandOPEN;
import org.openremote.controller.protocol.openwebnet.datastructure.command.DelayInterval;
...
import org.openremote.controller.protocol.openwebnet.queue.PriorityCommandQueue;
import org.openremote.controller.protocol.openwebnet.queue.PriorityQueueThread;
...
public class MyHomeJavaConnector
{
    ...
    public String readMonitoring() throws InterruptedException
    {
        ...
        do
        {
            ...
            catch(IOException e)
            {
                ...
                {
                    logger.error("Monitor disconnection problem. IOException: " + e1);
                }
                retry++;
                Thread.sleep(1000);
                logger.error("Monitor connection problem retry temptative: " + retry);
                try
                {
                    startMonitoring();
                }
                ...
                {
                    logger.error("Monitor connection problem. IOException: " + e1);
                }
                continue;
            }
        }
    ...
    /**
     * Reads messages from the monitor session and places them into the stack ({@link MonitorBuffer}) passed,
     * note: you always must call the method {@link startMonitoring()} before start reading from a monitor session.
     * This call is blocking on the socket but there is a timeout of 45s after that an exception is thrown.
     * The OpenWebNet protocol states that after 30s the connection is automatically closed, for this reason the monitor
     * socket periodically receive some keep-alive message.
     * In case of connection drop the socket timeout fires and this method tries to establish again the connection forever
     * notifying the attempt number.
     * @param monitorBuffer buffer in which to place the captured messages
     * @throws InterruptedException notify problem on sleep method
     */
    public void readMonitoring(MonitorBuffer monitorBuffer) throws InterruptedException
    {
        int retry = 0;
        do
        {
            try
            {
                ...
                do
                {
                    ...
                    if (ci == -1)
                    {
                        logger.error("Socket already closed by server");
                        inputStream.close();
                    ...
                    else
                    {
                        ...
                        else if (c == '#')              // Found second # command terminated correctly EXIT
                        {
                            ...
                            logger.info("Command: " + monitorBuffer.peek());
