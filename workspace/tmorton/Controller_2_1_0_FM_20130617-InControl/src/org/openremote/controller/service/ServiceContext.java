...
/**
 * This class defines an abstract service context without compile time links to
 * any particular service container implementation.
 * <p>
 *
 * Service context can be instantiated/registered only once per VM/classloader.
 * The subclasses are expected to register themselves directly from their
 * constructor via {@link #registerServiceContext(ServiceContext)} method. Only
 * one instance can be registered.
 * <p>
 *
 * The actual service provider is bound at runtime and can therefore vary
...
public abstract class ServiceContext
{
  ...
  // Constants ------------------------------------------------------------------------------------
    /**
     * Mutex for service context singleton to protect against multi-thread race
     * conditions.
     */
  ...
  // Enums ----------------------------------------------------------------------------------------
     ...
     *
     * This enum encapsulates the spring configuration specific bean names and
     * replaces them with typed enum identifiers to reduce sprinkling bean names
     * all over the codebase.
     *
     * These names are used where a direct lookup to spring context has been
     * made. Most of them should be ultimately unnecessary and to be replaced
     * with injecting dependent references instead (see various to-dos and JIRA
     * references below). Some others may still need to be (temporarily) added
     * to reduce direct API calls to SpringContext (see ORCJAVA-195)
     */
  ...
  // Class Members --------------------------------------------------------------------------------
    ...
    /**
     * TODO : See ORCJAVA-193 (http://jira.openremote.org/browse/ORCJAVA-193)
     * See ORCJAVA-173 (http://jira.openremote.org/browse/ORCJAVA-173)
     *
     *
     * @deprecated This is deprecated and should go away with refactoring of the
     * configuration to become part of deployment unit (see ORCJAVA-183) and
     * creating a REST admin interface for the controller (see ORCJAVA-173).
     */
    @Deprecated
    ...
    /**
     * TODO : currently used to access from REST servlet implementations --
     * should expose a single (deployer?) reference in WAR application context
     * that enables reference access to other services as necessary
     */
  ...
  // Abstract Methods -----------------------------------------------------------------------------
    /**
     * Returns a service implementation by the given service name. This is
     * customizable per different runtimes (Java SE, Android, etc.). This also
     * abstracts away compile-time dependencies to any particular bean-binding
     * or service frameworks. The concrete implementations are free to use
     * whichever framework or API mechanisms to retrieve and return the
     * requested service implementations.
     *
    ...
    /**
     * An explicit call made by the service context to the concrete
     * implementations allowing them to implement DI framework specific
     * initialization, if necessary.
     * <p>
     *
     * This in general is application (JVM) wide initialization, and is
     * therefore called once per controller's lifecycle -- should not be
     * confused with the deployment lifecycle which may occur multiple times
     * within controller process lifetime.
     */
