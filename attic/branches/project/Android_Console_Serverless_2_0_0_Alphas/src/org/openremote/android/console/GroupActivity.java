...
public class GroupActivity extends GenericActivity implements OnGestureListener, ORConnectionDelegate {
	...
	private void displayCurrentScreen() {
		...
		if (currentGroup == null || currentGroup.getScreens().size() == 0) {
			if (!useLocalCache) {
				ViewHelper.showAlertViewWithSetting(this, "No Group Found", "please download configuration from settings");
			}
		...
		if (screenSize == 0) {
			ViewHelper.showAlertViewWithTitle(this, "Info", "The group " + currentGroup.getName() + " has no " + (isLandscape ? "landscape" : "portrait") + " screen.");
			isLandscape = !isLandscape;
			screenSize = currentGroup.getScreenSizeByOrientation(isLandscape);
		}

		currentGroupView = new GroupView(this, currentGroup);
		groupViews.put(currentGroup.getGroupId(), currentGroupView);
		...
		UserCache.saveCurrentGroupIdAndScreenId(GroupActivity.this, currentGroupView.getGroup().getGroupId(),
