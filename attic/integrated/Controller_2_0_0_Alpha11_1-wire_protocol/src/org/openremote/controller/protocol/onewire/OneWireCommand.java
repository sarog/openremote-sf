...
/**
 * 1-wire protocol implementation for OpenRemote. Program accesses 1-wire network (owserver) via library jowfsclient.
 *
 * Parameters used:
 *
 ...
 * filename        or sensor attribute - is filename in owfs that holds values, such as "temperature", "temperature9",
 *                 "humidity" or similar
 * refreshTime     time interval in seconds between two calls to owserver; any request between these two calls will use
 *                 cached value. In fact, it is cache timeout value.
 *
...
//    private static Logger logger = Logger.getLogger(OneWireCommandBuilder.ONEWIRE_PROTOCOL_LOG_CATEGORY);
    private static Logger logger = Logger.getLogger(OneWireCommand.class);
    ...
    public String read(EnumSensorType sensorType, Map<String, String> stateMap) {
        ...
        if(timestamp > lastUpdateTimestamp+refreshTime) {
            ...
            if(null == value) {
                // we were not able to read value, so let's use old one + "!" to show that value is not up to date
            ...
            logger.debug("1-Wire sensor "+deviceAddress+"/"+filename+" value is still within cache timeout limit -> No update.");
            return lastValue;
        }
    }
